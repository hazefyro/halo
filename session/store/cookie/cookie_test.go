...
import (
	...
	"github.com/hazefyro/auth/session/store/cookie"
)

var signingKey = []byte("test-signing-key")

func newStore(t *testing.T, opts ...cookie.Option) *cookie.Store {
	t.Helper()
	allOpts := append([]cookie.Option{cookie.WithSigningKey(signingKey)}, opts...)
	store, err := cookie.New(allOpts...)
	if err != nil {
...
func TestNewRejectsInvalidTTL(t *testing.T) {
	store, err := cookie.New(cookie.WithSigningKey(signingKey), cookie.WithTTL(0))
...
func TestStoreTTL(t *testing.T) {
	store := newStore(t, cookie.WithTTL(30*time.Minute))
...
func TestStoreEncodeAndGetRoundTrip(t *testing.T) {
	...
	store := newStore(t, cookie.WithIssuer("auth-test"))
...
func TestStoreGetRejectsWrongIssuer(t *testing.T) {
	now := time.Now()
	issuerStore := newStore(t, cookie.WithIssuer("issuer-a"))
	token, err := issuerStore.Encode(testSession(now))
	if err != nil {
		t.Fatalf("Encode() error = %v", err)
	}

	store := newStore(t, cookie.WithIssuer("issuer-b"))
